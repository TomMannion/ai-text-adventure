...
const useFetchGameData = (setLoadingProgress: (progress: number) => void) => {
  ...
  const { chosenCharacter, chosenGenre, chosenImage, apiKey, provider, storyStart } = state;

  useEffect(() => {
    const fetchGameWorldAndCharacterInfo = async () => {
      if (chosenCharacter !== '' && storyStart === '') {
        const { characterTraits, characterBio, characterImage, characterGender } = await fetchCharacterTraitsAndBio(chosenGenre, chosenCharacter, chosenImage, apiKey, provider);
        ...
        const { storyStart, options } = await fetchStoryStart(chosenGenre, chosenCharacter, characterTraits, characterBio, characterGender, apiKey, provider );
        const storySummary = await fetchStorySummary(storyStart, apiKey, provider);
